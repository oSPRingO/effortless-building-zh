...
import nl.requios.effortlessbuildingzh.gui.RandomizerBagScreen;
import nl.requios.effortlessbuildingzh.gui.buildmode.PlayerSettingsGui;
import nl.requios.effortlessbuildingzh.gui.buildmode.RadialMenu;
import nl.requios.effortlessbuildingzh.gui.buildmodifier.ModifierSettingsGui;
import nl.requios.effortlessbuildingzh.helper.ReachHelper;
import nl.requios.effortlessbuildingzh.network.*;
import nl.requios.effortlessbuildingzh.render.ShaderHandler;
...
public class ClientProxy implements IProxy {
    ...
    public void clientSetup(FMLClientSetupEvent event) {
        ...
        DeferredWorkQueue.runLater( () -> ScreenManager.registerFactory(EffortlessBuildingZh.RANDOMIZER_BAG_CONTAINER, RandomizerBagScreen::new));
    }
    ...
    public static void onKeyPress(InputEvent.KeyInputEvent event) {
        ...
        if (keyBindings.length >= 8 && keyBindings[7].isPressed()) {
            ShaderHandler.init();
            EffortlessBuildingZh.log(player, "Reloaded shaders");
        }
    ...
    public static void openModifierSettings() {
        ...
        if (ReachHelper.getMaxReach(player) == 0) {
            EffortlessBuildingZh.log(player, "Build modifiers are disabled until your reach has increased. Increase your reach with craftable reach upgrades.");
        } else {
...
//        return player.rayTrace(raytraceRange, 1f, RayTraceFluidMode.NEVER);
    ...
    public void logTranslate(PlayerEntity player, String prefix, String translationKey, String suffix, boolean actionBar) {
        EffortlessBuildingZh.log(Minecraft.getInstance().player, prefix + I18n.format(translationKey) + suffix, actionBar);
    }
}
